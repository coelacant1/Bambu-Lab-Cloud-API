...
Tests for the Bambu Lab API proxy server functionality.
"""
...
class TestProxyTokenManager(unittest.TestCase):
    ...
    def test_token_listing(self):
        """Test token listing with masking"""
        print("Test: Token listing...", end=" ")
        tokens = self.token_manager.list_tokens()
        self.assertIn("custom_token_1", tokens)
        # Token manager masks tokens with first 4 chars + "..."
        masked = tokens["custom_token_1"]
        self.assertTrue(masked.startswith("real") or "..." in masked or "***" in masked)
        print("PASSED")
...
class TestProxyAdminEndpoints(unittest.TestCase):
    """Test admin endpoints"""
    
    def test_admin_endpoint_concept(self):
        """Test admin endpoint concept"""
        print("Test: Admin endpoint concept...", end=" ")
        # Admin endpoints should list tokens securely
        masked_token = "abc***"
        self.assertTrue(masked_token.endswith("***") or masked_token.endswith("..."))
        print("PASSED")
...
def run_tests():
    ...
    suite.addTests(loader.loadTestsFromTestCase(TestProxyAdminEndpoints))
